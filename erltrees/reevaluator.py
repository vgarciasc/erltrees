...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', '--episodes', help='Number of episodes to run when evaluating model', required=False,
                        default=10, type=int)
    parser.add_argument('--episodes_to_evaluate_best',
                        help='Number of episodes to run when evaluating best model at the end', required=False,
                        default=100, type=int)
    parser.add_argument('--norm_state', help="Should normalize state?", required=False, default=False,
    ...
    parser.add_argument('--should_prune_by_visits', help='Should prune trees by visits?', required=False, default=False,
                        type=lambda x: (str(x).lower() == 'true'))
    parser.add_argument('--should_print_tree', help='Should print tree?', required=False, default=False,
                        type=lambda x: (str(x).lower() == 'true'))
    parser.add_argument('--denormalize_thresholds', help='Should denormalize thresholds?', required=False,
    ...
    history = []
    command_line = "\n\npython -m erltrees.reevaluator " + " ".join(
    ...
    for i, string in enumerate(tree_strings):
        ...
        rl.collect_metrics(config, trees=[tree], alpha=args["alpha"],
                           ...
                           n_jobs=-1)
        print(
            f"Mean reward, std reward: {tree.reward} ± {tree.std_reward}, fitness: {tree.fitness}, SR: {tree.success_rate}")
    ...
    if args["should_prune_by_visits"]:
        ...
        print(
            f"  Average reward before pruning: {'{:.3f}'.format(np.mean(avg_rewards_before_pruning))} ± {'{:.3f}'.format(np.mean(std_rewards_before_pruning))}")
        print(
            f"  Average success rate before pruning: {'{:.3f}'.format(np.mean(success_rates_before_pruning))} ± {'{:.3f}'.format(np.std(success_rates_before_pruning))}")
        print()
        print("After pruning:")
        print(
            f"  Average reward: {'{:.3f}'.format(np.mean(avg_rewards_after_pruning))} ± {'{:.3f}'.format(np.mean(std_rewards_after_pruning))}")
        print(
            f"  Average success rate: {'{:.3f}'.format(np.mean(success_rates_after_pruning))} ± {'{:.3f}'.format(np.std(success_rates_after_pruning))}")
        print(f"  Average tree size: {'{:.3f}'.format(np.mean(tree_sizes))} ± {'{:.3f}'.format(np.std(tree_sizes))}")
    else:
        print(
            f"  Average reward: {'{:.3f}'.format(np.mean(avg_rewards_before_pruning))} ± {'{:.3f}'.format(np.mean(std_rewards_before_pruning))}")
        print(
            f"  Average success rate: {'{:.3f}'.format(np.mean(success_rates_before_pruning))} ± {'{:.3f}'.format(np.std(success_rates_before_pruning))}")
        print(f"  Average tree size: {'{:.3f}'.format(np.mean(tree_sizes))} ± {'{:.3f}'.format(np.std(tree_sizes))}")
